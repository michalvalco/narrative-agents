...
narrative-agents-repo/examples/memory_efficiency_viz.py (October 2025),
adapted to the current core: narrative agents now run the budgeted
selection rule (Experiment_Spec.md §6) with expected_n set to the run
size. Set SELECTION = "absolute" to reproduce the archived October 2025
behavior (the v2 0.7-threshold rule). Offline: no API calls.
"""
...
# Import visualization module (same directory)
from visualizations import create_full_benchmark_figure

SELECTION = "budgeted"      # "absolute" reproduces the archived v2 behavior
NUM_EXPERIENCES = 100
RANDOM_SEED = 26            # fixed so reported figures are reproducible


def generate_experiences(count: int = NUM_EXPERIENCES) -> List[Experience]:
...
def build_agents(num_experiences: int = NUM_EXPERIENCES) -> Dict[str, NarrativeAgent]:
    """The four benchmark agents, narrative pair under SELECTION."""
    return {
        'Narrative-Learner': NarrativeAgent(
            'NarrativeLearner', Telos.LEARNING, context_window=30,
            selection=SELECTION, expected_n=num_experiences),
        'Narrative-Performer': NarrativeAgent(
            'NarrativePerformer', Telos.PERFORMING, context_window=30,
            selection=SELECTION, expected_n=num_experiences),
...
def compare_memory_efficiency():
    ...
    collecting the visualization data as it goes.
    """
    print("=" * 70)
    print("MEMORY EFFICIENCY COMPARISON WITH VISUALIZATIONS")
    print(f"(selection rule: {SELECTION}, seed: {RANDOM_SEED})")
    print("=" * 70)
    ...
    agents = build_agents(num_experiences)
    ...
    # Selection-rule report (Experiment_Spec.md §6: efficiency is
    # selection under a budget, shown with the raw score distribution)
    print("\nSELECTION UNDER BUDGET (narrative agents):")
    print("-" * 70)
    for name, agent in agents.items():
        if 'Narrative' in name:
            stats = agent.memory_efficiency()
            s = stats['raw_score_summary']
            print(f"{name:<25} budget={stats['budget']:<4} floor={stats['floor']:<5} "
                  f"scores n={s['n']} min={s['min']:.2f} mean={s['mean']:.2f} max={s['max']:.2f}")
            print(f"{'':<25} core by type: {stats['core_composition_by_type']} "
                  f"displaced: {stats['displaced_count']}")
    ...
    create_full_benchmark_figure(viz_data, output_path)
